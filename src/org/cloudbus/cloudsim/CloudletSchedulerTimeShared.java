...
import dnabds.DNABDSCloudEncryption;
...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	public class CustomCloudletScheduler extends CloudletSchedulerTimeShared {
		...
		private DNABDSCloudEncryption dnaEncryption;
		private DNABDSCloudEncryption.CSPT cspt;

		public CustomCloudletScheduler() {
			this.dnaEncryption = new DNABDSCloudEncryption();
			this.cspt = dnaEncryption.new CSPT(); // Initialize CSPT for access management
		}
		...
		public double updateVmProcessing(double currentTime, List<Double> mipsShare) {
			...
			// Process each cloudlet and apply DNABDS encryption
			for (ResCloudlet rcl : getCloudletExecList()) {
				String cloudletData = "Sample data for Cloudlet " + rcl.getCloudletId(); // Mock data for example

				// Generate a unique key for the cloudlet based on user info and VM details
				String userId = Integer.toString(rcl.getCloudlet().getUserId());
				String password = "securePassword"; // Placeholder for a secure password source
				String macAddress = "MAC:00:1B:44:11:3A:B7";

				DNABDSCloudEncryption.DNAKeyGenerator keyGen = dnaEncryption.new DNAKeyGenerator(userId, password, macAddress);
				String secretKey = keyGen.generateSecretKey();

				// Check authorization in CSPT
				if (csptAuthorization(userId)) {
					// Apply DNA-based encryption
					DNABDSCloudEncryption.DNAEncryption dnaEnc = dnaEncryption.new DNAEncryption(secretKey.hashCode());
					String encryptedData = dnaEnc.encrypt(cloudletData);

					System.out.println("Encrypted Data for Cloudlet " + rcl.getCloudletId() + ": " + encryptedData);
				} else {
					System.out.println("Unauthorized access for Cloudlet " + rcl.getCloudletId() + " by User " + userId);
				}
			}
			return nextEvent;
		}

		private boolean csptAuthorization(String doid) {
			List<String> authorizedDOIDs = List.of("DOID1", "DOID2", "DOID3");
			List<List<String>> GIDList = List.of(List.of(), List.of());
			List<String> U = List.of("DOID4");

			// Insert DOID into CSPT for dynamic access management
			cspt.insertDO(doid, authorizedDOIDs, GIDList, U);
			return authorizedDOIDs.contains(doid);
		}
	}
}